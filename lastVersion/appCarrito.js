...
const mainCarrito = document.getElementById("mainCarrito"); // main carrito
            ...
            if(obj.talles[x].vendido === true){
                ...
                articuloCarrito.innerHTML = `<div class="boxCarrito__columnA">
                                            ...
                                            <div class="boxCarrito__columnB" id="contieneCantidad">
                                                <p class="desC">Cantidad: ${obj.talles[x].cantidad}</p>
                                                <p class="desC" >Talle: ${obj.talles[x].talle}</p>
                                            </div>                                                
                                            <div class="boxCarrito__columnC">
                                                <p class="desC" >Precio unidad: </p>
                                                <p class="subPrice"> $${obj.precioConImp} </p>
                                            </div>
                                            <button id="btnLimpiar${obj.id}${obj.talles[x].talle}" class="btnElimina fondoBoton btn-floating halfway-fab waves-effect waves-light" title="eliminar"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
                ...
                let btnLimpiar = document.getElementById(`btnLimpiar${obj.id}${obj.talles[x].talle}`);
                    ...
                    if((carroDeCompras.find((el)=> el.id == obj.id))){
                        ...
                        for(let i=0; i<elemento.talles.length; i++){
                            ...
                            if((i !== x)&&( elemento.talles[i].vendido )){
                                ...
                                console.log("quiero saber si es el elem correcto "+elemento.talles[i].cantidad);
...
btnComprar.addEventListener('click', ()=> {
    ...
    // swal("Compra realizada", `Usted compro por un monto de ${valor}`, 'success');
